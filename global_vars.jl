...
no_move = ones(Int32, no_birds+1) #An array which will allow us to keep track of which agents never move
new_pos::Vector{Tuple{Float64, Float64}} = [(0.0, 0.0) for i in 1:(no_birds+1)] #An array that will store the new positions of the agents for movement when we go to the model step
