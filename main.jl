...
selfish_lifetimes = Vector{Int32}(undef, 0)
collab_lifetimes = Vector{Int32}(undef, 0)
...
function model_step!(model)
	...
	for agent_i in all_agents_iterable
		...
		#print("The time for calculating a cell was\n")
                ...
                agent_i.delta_dod_var = abs(new_area - agent_i.tdod) - abs(agent_i.A - agent_i.tdod)
		agent_i.A = new_area
		...
		if(change[1] == 1)
			if(agent_i.collaborator == 1)
				push!(collab_lifetimes, agent_i.lifetime)
			else
				push!(selfish_lifetimes, agent_i.lifetime)
			end
			agent_i.lifetime = 1
