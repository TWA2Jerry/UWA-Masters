...
function initialise(; target_area_arg = 1000*sqrt(12), simulation_number_arg = 1, no_bird = 100, seed = 123, tracked_agent_arg = tracked_agent, no_moves_arg = no_birds, left_bias_arg = 0.5, q_arg = 8, qp_arg = 1, m_arg = 100, fov_arg = 180.0, area_args = (sqrt(12), 1000*sqrt(12)))
	...
	for i::Int32 in 1:no_birds
		...
		initial_cell::Vector{Tuple{Tuple{Float64, Float64}, Int64, Int64}} = Vector{Tuple{Tuple{Float64, Float64}, Int64, Int64}}(undef, 0)
		...
		elseif(abs(model.fov - 360.0) < eps)
			initial_cell = @time voronoi_cell_bounded(model, ri, neighbouring_positions, rho, eps, inf, temp_hp, initial_vels[i])
		else
...
function model_step!(model)
	...
	for agent_i in all_agents_iterable
		...
		#print("The time for calculating a cell was\n")
                new_cell_i::Vector{Tuple{Tuple{Float64, Float64}, Int64, Int64}} = Vector{Tuple{Tuple{Float64, Float64}, Int64, Int64}}(undef, 0)
				if(abs(model.fov - 180.0) < eps)
					new_cell_i = voronoi_cell_bounded(model, ri, neighbour_positions, rho, eps, inf, temp_hp, agent_i.vel, [relic_half_plane])
				elseif(abs(model.fov - 360.0) < eps)
					new_cell_i = voronoi_cell_bounded(model, ri, neighbour_positions, rho, eps, inf, temp_hp, agent_i.vel)	
				else
					new_cell_i = voronoi_cell_bounded(model, ri, neighbour_positions, rho, eps, inf, temp_hp, agent_i.vel, [left_half_plane, right_half_plane])	
				end
		...
		if(agent_i.no_neighbours > 40) 
			...
			f = draw_cell(true_new_cell_i)
			save("./Debug_Images/cell.pdf", f)
			f = return_thesis_figures(model)
			save("./Debug_Images/model.pdf", f)
			#exit() 
		end
