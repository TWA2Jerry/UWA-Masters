...
function run_ABM()
	...
	ncompac_frac_file = open("compaction_frac.txt", "r")
	nmean_a_file = open("mean_area.txt", "r")
	nrot_o_file = open("rot_order.txt", "r")
...
cf_lines = readlines(ncompac_frac_file)
ma_lines = readlines(nmean_a_file)
rot_o_lines = readlines(nrot_o_file)

print("The first thing read from the ncompac_frac_file was $(cf_lines[1])\n")
