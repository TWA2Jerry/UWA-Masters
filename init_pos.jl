...
function init_thesis(cellx, celly, no_agents, spacexspan, spaceyspan, offsetx, offsety, initial_positions, initial_vels)
	...
	push!(initial_positions, ro .+ (cos(2*pi/3)*150, sin(2*pi/3)*150))
	push!(initial_positions, ro .+ (190.0, 0.0))
	push!(initial_positions, ro .+ (cos(70*pi/180)*160, sin(70*pi/180)*160))
	push!(initial_positions, ro .+ (cos(3*pi/4)*190, sin(3*pi/4)*190))
	push!(initial_positions, ro .+ (cos(85*pi/180)*180, sin(85*pi/180)*180))	
